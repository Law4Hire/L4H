...
-- Re-run table creation since it seems to be missing or rolled back
...
BEGIN
    ...
    BEGIN
        ...
        VALUES (
            'Denise Cann', 'Managing Attorney', 'Experienced immigration attorney.', @UserEmail, '555-0100', '["JD"]', '["Immigration"]', '["English"]', '555-0101', @UserEmail, 'Main Office', 350.00, 1, 1, 1, GETUTCDATE(), GETUTCDATE(), 0
        );
        SET @AttorneyId = SCOPE_IDENTITY();
        PRINT 'Created Attorney ID: ' + CAST(@AttorneyId AS VARCHAR(10));
    END
    ELSE
    BEGIN
        PRINT 'Using Existing Attorney ID: ' + CAST(@AttorneyId AS VARCHAR(10));
    END

    -- Link User to Attorney
    UPDATE AspNetUsers SET AttorneyId = @AttorneyId WHERE Id = @UserId;
    PRINT 'Linked User to Attorney.';
END
ELSE
BEGIN
