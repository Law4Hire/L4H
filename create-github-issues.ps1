# PowerShell script to create GitHub issues for the new interview system
# Run this with: .\create-github-issues.ps1

Write-Host "Creating GitHub issues for new interview system..." -ForegroundColor Cyan
Write-Host ""

# Issue 1: API Endpoints
Write-Host "Creating Issue #1: API Endpoints..." -ForegroundColor Yellow
gh issue create `
  --title "API Endpoints: Update InterviewController for new eligibility service" `
...
Write-Host "Creating Issue #2: Interview Wizard..." -ForegroundColor Yellow
gh issue create `
  --title "Frontend: Interview Wizard Component with Cookie State" `
  --label "frontend,enhancement,high-priority" `
  --body @"
## Overview
...
Write-Host "Creating Issue #3: Results Page..." -ForegroundColor Yellow
gh issue create `
  --title "Frontend: Visa Results Page with Eligible/Potential Distinction" `
  --label "frontend,enhancement,high-priority" `
  --body @"
## Overview
...
Write-Host "Creating Issue #4: Single-Page Registration..." -ForegroundColor Yellow
gh issue create `
  --title "Frontend: Single-Page Registration Form (Mobile Optimized)" `
  --label "frontend,enhancement,high-priority" `
  --body @"
## Overview
...
Write-Host "Creating Issue #5: Attorney Lock-In..." -ForegroundColor Yellow
gh issue create `
  --title "Attorney Lock-In Workflow and UI" `
  --label "frontend,backend,enhancement,medium-priority" `
  --body @"
## Overview
...
Write-Host "Creating Issue #6: Dashboard Updates..." -ForegroundColor Yellow
gh issue create `
  --title "Dashboard Updates: Show Interview Results and Retake Option" `
  --label "frontend,enhancement,medium-priority" `
  --body @"
## Overview
...
Write-Host "Creating Issue #7: Localization..." -ForegroundColor Yellow
gh issue create `
  --title "Localization: Translate Interview Content to 22 Languages" `
  --label "i18n,enhancement,low-priority" `
  --body @"
## Overview
...
# Issue 8: Testing
Write-Host "Creating Issue #8: Testing..." -ForegroundColor Yellow
gh issue create `
  --title "Testing: Comprehensive Test Coverage for Interview System" `
  --label "testing,quality,medium-priority" `
  --body @"
## Overview
...
Write-Host ""
Write-Host "View issues at: https://github.com/Law4Hire/L4H/issues" -ForegroundColor Cyan
