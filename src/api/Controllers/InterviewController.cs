...
public class InterviewController : ControllerBase
{
    ...
    private readonly DecisionTreeVisaEligibilityService _eligibilityService;
    ...
    public InterviewController(
        ...
        DecisionTreeVisaEligibilityService eligibilityService,
    ...
    /// <summary>
    /// Start a new unauthenticated interview session (cookie-based)
    /// </summary>
    /// <param name="request">Unauthenticated interview start request</param>
    /// <returns>Interview session details with cookie information</returns>
    [HttpPost("unauth/start")]
    [AllowAnonymous]
    [ProducesResponseType<UnauthInterviewStartResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> UnauthStart([FromBody] UnauthInterviewStartRequest request)
    {
        try
        {
            _logger.LogInformation("Starting unauthenticated interview session");

            // Create anonymous interview session (no user or case)
            var session = new InterviewSession
            {
                Id = Guid.NewGuid(),
            ...
            _logger.LogInformation("Unauthenticated interview session {SessionId} created", session.Id);

            var expiresAt = DateTime.UtcNow.AddHours(24);

            return Ok(new UnauthInterviewStartResponse
            {
                SessionId = session.Id,
                CookieKey = "l4h_interview_progress",
                ExpiresAt = expiresAt
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error starting unauthenticated interview");
            return StatusCode(500, new ProblemDetails
            {
                Title = _localizer["Common.InternalError"],
                Detail = _localizer["Common.InternalErrorDetail"],
                Instance = "/api/v1/interview/unauth/start"
            });
        }
    }

    /// <summary>
    /// Submit an answer for unauthenticated interview (cookie-based)
    /// </summary>
    /// <param name="request">Unauthenticated interview answer request</param>
    /// <returns>Answer confirmation</returns>
    [HttpPost("unauth/answer")]
    [AllowAnonymous]
    [ProducesResponseType<InterviewAnswerResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status400BadRequest)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> UnauthAnswer([FromBody] UnauthInterviewAnswerRequest request)
    {
        try
        {
            _logger.LogInformation("Recording unauthenticated answer for session {SessionId}: {QuestionKey} = {AnswerValue}",
            ...
            if (string.IsNullOrWhiteSpace(request.QuestionKey))
            {
                return BadRequest(new ProblemDetails
                {
                    ...
                    Instance = "/api/v1/interview/unauth/answer"
                });
            }
            ...
            if (session.StartedAt.AddHours(24) < DateTime.UtcNow)
            {
                _logger.LogInformation("Session {SessionId} has expired", request.SessionId);
                session.Status = "expired";
                session.FinishedAt = DateTime.UtcNow;
                await _context.SaveChangesAsync().ConfigureAwait(false);
        ...
        {
            _logger.LogError(ex, "Error recording unauthenticated answer for session {SessionId}", request.SessionId);
            return StatusCode(500, new ProblemDetails
            {
                Title = _localizer["Common.InternalError"],
                Detail = _localizer["Common.InternalErrorDetail"],
    ...
    /// <summary>
    /// Get multi-visa eligibility results for a completed interview session
    /// </summary>
    /// <param name="request">Get visa results request</param>
    /// <returns>Multi-visa eligibility results with eligible and potential visas</returns>
    [HttpPost("results")]
    [AllowAnonymous]
    [ProducesResponseType<MultiVisaEligibilityResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status400BadRequest)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetVisaResults([FromBody] GetVisaResultsRequest request)
    {
        try
        {
            _logger.LogInformation("Getting visa eligibility results for session {SessionId}", request.SessionId);
            ...
            if (session == null)
            {
                return NotFound(new ProblemDetails
                {
                    Title = _localizer["Interview.SessionNotFound"],
                    Detail = _localizer["Interview.SessionNotFoundDetail"],
                    Instance = $"/api/v1/interview/results"
                });
            }

            // Check if results already exist for this session
            var existingResults = await _context.VisaEligibilityResults
                .Include(r => r.VisaType)
                .Where(r => r.InterviewSessionId == request.SessionId)
                .ToListAsync()
                .ConfigureAwait(false);

            MultiVisaEligibilityResponse response;

            if (existingResults.Any())
            {
                _logger.LogInformation("Using existing eligibility results for session {SessionId}", request.SessionId);

                // Return existing results
                response = new MultiVisaEligibilityResponse
                {
                    SessionId = request.SessionId,
                    EligibleVisas = existingResults
                        .Where(r => r.EligibilityStatus == "Eligible")
                        .OrderByDescending(r => r.MatchScore)
                        .Select(r => new VisaEligibilityDto
                        {
                            VisaCode = r.VisaType.Code,
                            VisaName = r.VisaType.Name,
                            EligibilityStatus = r.EligibilityStatus,
                            MatchScore = r.MatchScore,
                            Rationale = r.Rationale ?? "",
                            MetRequirements = string.IsNullOrEmpty(r.MetRequirements)
                                ? new List<string>()
                                : r.MetRequirements.Split('|', StringSplitOptions.RemoveEmptyEntries).ToList(),
                            UnmetRequirements = string.IsNullOrEmpty(r.UnmetRequirements)
                                ? new List<string>()
                                : r.UnmetRequirements.Split('|', StringSplitOptions.RemoveEmptyEntries).ToList()
                        }).ToList(),
                    PotentialVisas = existingResults
                        .Where(r => r.EligibilityStatus == "Potential")
                        .OrderByDescending(r => r.MatchScore)
                        .Select(r => new VisaEligibilityDto
                        {
                            VisaCode = r.VisaType.Code,
                    ...
                    TotalEvaluated = existingResults.Count,
                    EvaluatedAt = existingResults.Max(r => r.CreatedAt)
                };
            }
            else
            {
                _logger.LogInformation("Evaluating visa eligibility for session {SessionId}", request.SessionId);

                // Build answers dictionary
                var answers = session.QAs.ToDictionary(qa => qa.QuestionKey, qa => qa.AnswerValue);

                // Evaluate eligibility using the new service
                var eligibilityResults = await _eligibilityService.EvaluateEligibilityAsync(answers, session.User)
                    .ConfigureAwait(false);

                _logger.LogInformation("Evaluation complete for session {SessionId}: {TotalResults} results, {EligibleCount} eligible, {PotentialCount} potential",
                    request.SessionId, eligibilityResults.Count,
                    eligibilityResults.Count(r => r.EligibilityStatus == "Eligible"),
                    eligibilityResults.Count(r => r.EligibilityStatus == "Potential"));

                // Save results to database
                foreach (var result in eligibilityResults)
                {
                    result.InterviewSessionId = request.SessionId;
                    _context.VisaEligibilityResults.Add(result);
                }
                ...
                // Load visa types for response
                var visaTypeIds = eligibilityResults.Select(r => r.VisaTypeId).Distinct().ToList();
                var visaTypes = await _context.VisaTypes
                    .Where(vt => visaTypeIds.Contains(vt.Id))
                    .ToDictionaryAsync(vt => vt.Id, vt => vt)
                    .ConfigureAwait(false);
                ...
                {
                    SessionId = request.SessionId,
                    EligibleVisas = eligibilityResults
                        .Where(r => r.EligibilityStatus == "Eligible")
                        .OrderByDescending(r => r.MatchScore)
                        .Select(r => new VisaEligibilityDto
                        {
                            VisaCode = visaTypes[r.VisaTypeId].Code,
                            VisaName = visaTypes[r.VisaTypeId].Name,
                            EligibilityStatus = r.EligibilityStatus,
                            MatchScore = r.MatchScore,
                            Rationale = r.Rationale ?? "",
                            MetRequirements = string.IsNullOrEmpty(r.MetRequirements)
                    ...
                    PotentialVisas = eligibilityResults
                        .Where(r => r.EligibilityStatus == "Potential")
                        .OrderByDescending(r => r.MatchScore)
                        .Select(r => new VisaEligibilityDto
                        {
                    ...
                    TotalEvaluated = eligibilityResults.Count,
                    EvaluatedAt = DateTime.UtcNow
                };
            }

            _logger.LogInformation("Returning {EligibleCount} eligible visas and {PotentialCount} potential visas for session {SessionId}",
                response.EligibleVisas.Count, response.PotentialVisas.Count, request.SessionId);

            return Ok(response);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting visa eligibility results for session {SessionId}", request.SessionId);
            return StatusCode(500, new ProblemDetails
            {
                Title = _localizer["Common.InternalError"],
                Detail = _localizer["Common.InternalErrorDetail"],
    ...
    /// <summary>
    /// Lock a specific visa type selection for a case (Attorney feature)
    /// </summary>
    /// <param name="request">Lock visa selection request</param>
    /// <returns>Lock confirmation with visa details</returns>
    [HttpPost("lock-visa-selection")]
    [ProducesResponseType<LockVisaSelectionResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status400BadRequest)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> LockVisaSelection([FromBody] LockVisaSelectionRequest request)
    {
        try
        {
            var userId = GetCurrentUserId();
            _logger.LogInformation("Locking visa selection for case {CaseId} by user {UserId}", request.CaseId, userId);
            ...
            if (caseEntity == null)
            {
                return NotFound(new ProblemDetails
                {
                    Title = "Case Not Found",
                    Detail = _localizer["Cases.NotFound"],
                    Instance = "/api/v1/interview/lock-visa-selection"
                });
            }

            // Get the visa type
            var visaType = await _context.VisaTypes
                .FindAsync(request.VisaTypeId)
                .ConfigureAwait(false);
            ...
            if (visaType == null)
            {
                return BadRequest(new ProblemDetails
                {
                    ...
                    Detail = "The specified visa type does not exist",
                    Instance = "/api/v1/interview/lock-visa-selection"
                });
            }

            // Lock the visa selection
            caseEntity.AttorneySelectedVisaTypeId = request.VisaTypeId;
            caseEntity.IsVisaLockedByAttorney = true;
            caseEntity.VisaLockedAt = DateTime.UtcNow;
            caseEntity.VisaLockedByStaffId = userId.Value;
            caseEntity.LastActivityAt = DateTimeOffset.UtcNow;

            await _context.SaveChangesAsync().ConfigureAwait(false);

            _logger.LogInformation("Visa selection locked for case {CaseId}: {VisaCode} ({VisaName})",
                request.CaseId, visaType.Code, visaType.Name);

            return Ok(new LockVisaSelectionResponse
            {
                Success = true,
                VisaCode = visaType.Code,
                VisaName = visaType.Name,
                LockedAt = caseEntity.VisaLockedAt!.Value,
                LockedByStaffId = userId.Value
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error locking visa selection for case {CaseId}", request.CaseId);
            return StatusCode(500, new ProblemDetails
            {
                Title = _localizer["Common.InternalError"],
                Detail = _localizer["Common.InternalErrorDetail"],
    ...
    /// <summary>
    /// Unlock visa selection for a case (Admin feature)
    /// </summary>
    /// <param name="request">Unlock visa selection request</param>
    /// <returns>Success response</returns>
    [HttpPost("unlock-visa-selection")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> UnlockVisaSelection([FromBody] UnlockVisaSelectionRequest request)
    {
        try
        {
            var userId = GetCurrentUserId();
            _logger.LogInformation("Unlocking visa selection for case {CaseId} by user {UserId}. Reason: {Reason}",
                request.CaseId, userId, request.Reason);
            ...
            if (caseEntity == null)
            {
                return NotFound(new ProblemDetails
                {
                    Title = "Case Not Found",
                    Detail = _localizer["Cases.NotFound"],
                    Instance = "/api/v1/interview/unlock-visa-selection"
                });
            }

            // Unlock the visa selection
            caseEntity.AttorneySelectedVisaTypeId = null;
            caseEntity.IsVisaLockedByAttorney = false;
            caseEntity.VisaLockedAt = null;
            caseEntity.VisaLockedByStaffId = null;
            caseEntity.LastActivityAt = DateTimeOffset.UtcNow;

            await _context.SaveChangesAsync().ConfigureAwait(false);

            _logger.LogInformation("Visa selection unlocked for case {CaseId}", request.CaseId);

            return NoContent();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error unlocking visa selection for case {CaseId}", request.CaseId);
            return StatusCode(500, new ProblemDetails
            {
                Title = _localizer["Common.InternalError"],
                Detail = _localizer["Common.InternalErrorDetail"],
    ...
    /// <summary>
    /// Check if a case has a locked visa selection
    /// </summary>
    /// <param name="request">Check lock status request</param>
    /// <returns>Lock status information</returns>
    [HttpPost("check-lock-status")]
    [ProducesResponseType<CheckLockStatusResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> CheckLockStatus([FromBody] CheckLockStatusRequest request)
    {
        try
        {
            var userId = GetCurrentUserId();
            _logger.LogInformation("Checking lock status for case {CaseId}", request.CaseId);

            // Get the case with visa type and staff details
            var caseEntity = await _context.Cases
                .Include(c => c.AttorneySelectedVisaType)
            ...
            if (caseEntity == null)
            {
                return NotFound(new ProblemDetails
                {
                    Title = "Case Not Found",
                    Detail = _localizer["Cases.NotFound"],
                    Instance = "/api/v1/interview/check-lock-status"
                });
            }

            CheckLockStatusResponse response;

            if (caseEntity.IsVisaLockedByAttorney && caseEntity.AttorneySelectedVisaType != null)
            {
                // Get staff name if locked
                string? staffName = null;
                if (caseEntity.VisaLockedByStaffId.HasValue)
                {
                    var staff = await _context.Users
                        .FindAsync(new UserId(caseEntity.VisaLockedByStaffId.Value))
                        .ConfigureAwait(false);
                    staffName = staff != null ? $"{staff.FirstName} {staff.LastName}" : "Unknown";
                }

                response = new CheckLockStatusResponse
                {
                    IsLocked = true,
                    VisaCode = caseEntity.AttorneySelectedVisaType.Code,
                    VisaName = caseEntity.AttorneySelectedVisaType.Name,
        ...
        {
            _logger.LogError(ex, "Error checking lock status for case {CaseId}", request.CaseId);
            return StatusCode(500, new ProblemDetails
            {
                Title = _localizer["Common.InternalError"],
                Detail = _localizer["Common.InternalErrorDetail"],
