...
const CannlawI18nContext = createContext<CannlawI18nContextType | undefined>(undefined)
...
export function CannlawI18nProvider({ 
  children, 
  additionalNamespaces = [],
  preloadNamespaces = ['common', 'errors', 'auth'],
  accessibility = {}
}: CannlawI18nProviderProps) {
  ...
  const loadNamespace = async (namespace: CannlawNamespace | SharedNamespace) => {
    try {
  ...
  const preloadNamespacesFunc = async (namespaces: (CannlawNamespace | SharedNamespace)[]) => {
    try {
  ...
  if (!i18nInitialized) {
    return <div>Loading Cannlaw translations...</div>
  }
  ...
  return (
    <I18nextProvider i18n={i18n}>
      <CannlawI18nContext.Provider value={value}>
        {children}
        {showNotification && (
...
export function useCannlawT(namespace?: CannlawNamespace | SharedNamespace) {
