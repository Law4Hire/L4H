 ...
 */
export function useRTLAccessibility(options: RTLAccessibilityOptions = {}): RTLAccessibilityReturn {
  const {
    manageFocusOrder = true,
    adjustKeyboardNavigation = true,
    announceDirectionChanges = true,
    customFocusOrder = [],
    excludeFromFocusManagement = []
  } = options
  
  const { i18n } = useTranslation()
  const previousDirectionRef = useRef<'ltr' | 'rtl'>('ltr')
  ...
  const getNavigationDirection = useCallback((key: string): 'forward' | 'backward' | 'up' | 'down' | null => {
    switch (key) {
      case 'ArrowLeft':
  ...
  const getRTLAriaAttributes = useCallback(() => ({
    dir: textDirection,
    ...(textDirection === 'rtl' && { 'aria-orientation': 'horizontal' as const })
  }), [textDirection])
  
  // Set up RTL-aware focus management for a container
  const setupRTLFocusManagement = useCallback((container: HTMLElement) => {
    if (!manageFocusOrder) return () => {}
    ...
    const updateFocusOrder = () => {
      ...
      const filteredElements = focusableElements.filter(element => {
        return !excludeFromFocusManagement.some(selector => 
      ...
      if (customFocusOrder.length > 0) {
        ...
        const remainingElements = filteredElements.filter(el => 
          !orderedElements.includes(el)
        )
      ...
      const handleKeyDown = (event: KeyboardEvent) => {
        if (!adjustKeyboardNavigation) return
  ...
  // Handle RTL-aware keyboard navigation
  const handleRTLKeyboardNavigation = useCallback((event: KeyboardEvent, container: HTMLElement): boolean => {
    if (!adjustKeyboardNavigation) return false
