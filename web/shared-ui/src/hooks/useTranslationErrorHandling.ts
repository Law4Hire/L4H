...
export function useTranslationErrorHandling(
  language?: string,
  namespace?: string,
  options: UseTranslationErrorHandlingOptions = {}
) {
  ...
  const [state, setState] = useState<TranslationErrorHandlingState>(() => ({
  ...
  useEffect(() => {
    const handleLoadingStart = (lng: string, ns: string) => {
      if (lng === currentLanguage && (!namespace || ns === currentNamespace)) {
        translationErrorHandler.startLoading(lng, ns)
      }
    }

    const handleLoadingSuccess = (lng: string, ns: string) => {
      if (lng === currentLanguage && (!namespace || ns === currentNamespace)) {
        translationErrorHandler.recordSuccess(lng, ns)
      }
    }

    const handleLoadingError = (lng: string, ns: string, msg: string) => {
      if (lng === currentLanguage && (!namespace || ns === currentNamespace)) {
  ...
  const retry = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const success = await translationErrorHandler.retryLoading(
        currentLanguage,
        currentNamespace,
        async (lang, ns) => {
  ...
  const getFallbackTranslation = useCallback((key: string, ns?: string): string | null => {
    try {
