...
export function useFallbackTranslation(
  key: string,
  defaultValue?: string,
  options: FallbackTranslationOptions = {}
) {
  const { t, i18n } = useTranslation()
  const [fallbackState, setFallbackState] = useState<FallbackTranslationState>({
  ...
  const getTranslation = useCallback((
    ...
    try {
      ...
      if (i18n.language !== fallbackLanguage) {
        try {
          const fallbackTranslation = i18n.getFixedT(fallbackLanguage, fallbackNamespace)(
            translationKey, 
   ...
   */
  const hasTranslation = useCallback((translationKey: string): boolean => {
    try {
   ...
   */
  const getMultipleTranslations = useCallback((
    keys: string[],
    fallbackValues?: { [key: string]: string }
  ): { [key: string]: string } => {
    const results: { [key: string]: string } = {}
    
    keys.forEach(key => {
      results[key] = getTranslation(
        key, 
        fallbackValues?.[key], 
        undefined
      )
 ...
 */
function extractReadableFromKey(key: string): string {
 ...
 */
export function withFallbackTranslation<T extends Record<string, any>>(
  Component: React.ComponentType<T & { fallbackTranslation: ReturnType<typeof useFallbackTranslation> }>,
  fallbackOptions?: FallbackTranslationOptions
): React.ComponentType<T> {
  return function FallbackTranslationWrapper(props: T) {
