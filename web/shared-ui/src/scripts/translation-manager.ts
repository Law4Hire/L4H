...
class TranslationManager {
  ...
  private languageManager: NewLanguageManager;
  private translationUpdater: SharedTranslationUpdater;
  private keyMigrator: TranslationKeyMigrator;
  private validator: TranslationCompletenessValidator;
   ...
   */
  async removeLanguage(languageCode: string): Promise<OperationResult> {
  ...
  async updateTranslations(
    ...
    try {
      ...
      // Update translations
      const result = await this.translationUpdater.updateTranslationKey(
        namespace,
        key,
        translations,
        {
   ...
   */
  async validateAll(): Promise<OperationResult> {
    ...
    try {
      ...
      const summary = {
        ...
        totalMissingKeys: validationReport.results.reduce((sum: number, r: any) => sum + r.missingKeys.length, 0),
      ...
      const incompleteLanguages = validationReport.results.filter((r: any) => r.completeness < 100);
      if (incompleteLanguages.length > 0) {
        console.log('\n⚠️  Incomplete Languages:');
        incompleteLanguages.forEach((result: any) => {
   ...
   */
  async generateReport(outputPath?: string): Promise<OperationResult> {
    ...
    try {
      ...
      const report = {
        ...
        summary: {
          totalLanguages: validationReport.summary.totalLanguages,
          completeLanguages: validationReport.results.filter((r: any) => r.completeness === 100).length,
          averageCompletion: validationReport.summary.averageCompleteness
        },
        languages: validationReport.results.map((result: any) => ({
  ...
  private generateRecommendations(results: any[]): string[] {
    ...
    const incompleteLanguages = results.filter((r: any) => r.completeness < 100);
    const averageCompletion = results.reduce((sum: number, r: any) => sum + r.completeness, 0) / results.length;
    ...
    const languagesWithManyMissing = results.filter((r: any) => r.missingKeys.length > 50);
    if (languagesWithManyMissing.length > 0) {
      recommendations.push(`Priority attention needed for languages with 50+ missing keys: ${languagesWithManyMissing.map((r: any) => r.language).join(', ')}`);
    }
