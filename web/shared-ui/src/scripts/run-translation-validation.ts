...
class TranslationValidationRunner {
   ...
   */
  async runAllValidations(): Promise<CombinedReport> {
   ...
   */
  private generateCombinedReport(results: ValidationSuite): CombinedReport {
    const totalIssues = 
      (results.consistency?.summary.totalIssues || 0) + 
      (results.interpolation?.summary.totalIssues || 0) +
      (results.completeness?.results?.filter((r: any) => r.completeness < 100).length || 0);

    const criticalIssues = 
      (results.consistency?.summary.criticalIssues || 0) + 
      (results.interpolation?.summary.criticalIssues || 0) +
      (results.completeness?.results?.filter((r: any) => r.completeness < 50).length || 0);
   ...
   */
  private calculateConsistencyScore(consistencyReport: any): number {
    if (!consistencyReport) return 0;
   ...
   */
  private calculateInterpolationScore(interpolationReport: any): number {
    if (!interpolationReport) return 0;
   ...
   */
  private determineHealthStatus(overallScore: number, criticalIssues: number): CombinedReport['healthStatus'] {
   ...
   */
  private generatePrioritizedActions(results: ValidationSuite, criticalIssues: number): string[] {
    ...
    if (results.completeness?.results) {
      const incompleteLanguages = results.completeness.results
        .filter((r: any) => r.completeness < 80)
    ...
    if (results.completeness?.recommendations) {
      actions.push(...results.completeness.recommendations.slice(0, 2).map((r: string) => `📋 ${r}`));
    }

    if (results.consistency?.recommendations) {
      actions.push(...results.consistency.recommendations.slice(0, 2).map((r: string) => `🔄 ${r}`));
    }

    if (results.interpolation?.recommendations) {
      actions.push(...results.interpolation.recommendations.slice(0, 2).map((r: string) => `🛠️  ${r}`));
    }
   ...
   */
  async saveCombinedReport(report: CombinedReport, outputPath?: string): Promise<string> {
   ...
   */
  async saveIndividualReports(results: ValidationSuite): Promise<void> {
