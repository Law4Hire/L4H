...
class SharedTranslationUpdater {
  ...
  private sharedLocalesPath: string;
  ...
  private sharedNamespaces = ['common', 'errors', 'forms', 'auth'];
   ...
   */
  async bulkUpdateTranslations(updates: TranslationUpdate[]): Promise<BulkUpdateResult> {
   ...
   */
  async synchronizeNamespace(namespace: string): Promise<UpdateResult> {
    ...
    for (const language of this.supportedLanguages) {
      if (language === 'en-US') continue; // Skip base language
   ...
   */
  private hasNestedValue(obj: any, key: string): boolean {
   ...
   */
  private validateStructure(translation: any, base: any): boolean {
   ...
   */
  async loadUpdatesFromFile(filePath: string): Promise<TranslationUpdate[]> {
    if (!fs.existsSync(filePath)) {
  ...
  generateUpdateTemplate(outputPath: string): void {
    const template: TranslationUpdate[] = [
      {
        key: 'example.newKey',
