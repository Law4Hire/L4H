...
class TranslationCompletenessValidator {
   ...
   */
  async validateAll(): Promise<CompletenessReport> {
   ...
   */
  private async validateApplication(localesPath: string, appName: string): Promise<ValidationResult[]> {
    ...
    for (const language of this.supportedLanguages) {
      ...
      for (const namespace of namespaces) {
        const result = await this.validateNamespace(
          languagePath,
          namespace,
   ...
   */
  private loadNamespaceTranslations(languagePath: string, namespaces: string[]): Record<string, TranslationFile> {
   ...
   */
  private flattenKeys(obj: TranslationFile, prefix = ''): string[] {
    const keys: string[] = [];
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === 'object' && value !== null) {
        keys.push(...this.flattenKeys(value as TranslationFile, fullKey));
      } else {
        keys.push(fullKey);
      }
    }
   ...
   */
  private validateInterpolation(translation: TranslationFile, baseTranslation: TranslationFile): string[] {
    const errors: string[] = [];
   ...
   */
  private generateReport(results: ValidationResult[]): CompletenessReport {
    ...
    const criticalIssues = results.filter(r => 
      r.completeness < 80 || r.interpolationErrors.length > 0
    ).length;
   ...
   */
  async saveReport(report: CompletenessReport, outputPath?: string): Promise<string> {
