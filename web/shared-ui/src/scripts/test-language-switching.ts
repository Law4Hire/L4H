...
class LanguageSwitchingTestRunner {
  ...
  private async testLanguage(language: string): Promise<TestResult> {
      ...
      for (const path of testPaths) {
        try {
          ...
          if (path.includes('common.json')) {
            if (data.welcome) translationSamples['common.welcome'] = data.welcome;
            if (data.loading) translationSamples['common.loading'] = data.loading;
            if (data.error) translationSamples['common.error'] = data.error;
          }
          
          if (path.includes('errors.json')) {
            if (data.network_error) translationSamples['errors.network_error'] = data.network_error;
          }
