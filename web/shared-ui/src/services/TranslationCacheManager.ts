...
export interface CacheConfig {
  ...
  criticalTTL: number // TTL for critical namespaces
...
export class TranslationCacheManager {
  ...
  clear(options?: {
    ...
    for (const [key, entry] of this.cache.entries()) {
      ...
      if (options?.olderThan && (now - entry.timestamp) < options.olderThan) continue
  ...
  private getTTL(namespace: string, priority: CacheEntry['priority']): number {
  ...
  private loadFromStorage(): void {
    ...
    try {
      ...
      const now = Date.now()
      for (const [key, entry] of Object.entries(entries as { [key: string]: CacheEntry })) {
        if (entry.expiresAt > now) {
          this.cache.set(key, entry)
        }
      }
