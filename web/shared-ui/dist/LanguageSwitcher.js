...
export function LanguageSwitcher({ variant = 'full', className, 'aria-label': ariaLabel = 'Select language', 'aria-describedby': ariaDescribedBy, }) {
    ...
    if (isLoading) {
        return (_jsx("div", { className: "px-3 py-2 text-sm text-gray-500 dark:text-gray-400", children: t('common.loading') }));
    }
    return (_jsx("select", { value: currentCulture, onChange: handleLanguageChange, "aria-label": ariaLabel, "aria-describedby": ariaDescribedBy, className: clsx('border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500', variant === 'compact' ? 'px-2 py-1 text-sm' : 'px-3 py-2 text-base', className), children: cultures.map((culture) => (_jsx("option", { value: culture.code, children: culture.displayName }, culture.code))) }));
}
