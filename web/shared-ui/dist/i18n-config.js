...
export function setRTLDirection(language) {
    if (typeof document !== 'undefined') {
...
export function getTextAlign(language, align = 'start') {
    if (align === 'center')
        return 'center';
    const isRTLLang = isRTL(language);
    switch (align) {
...
export function formatCurrency(value, language, currency = 'USD', options) {
    try {
...
export function formatPercentage(value, language, options) {
    try {
...
export function formatRelativeTime(date, language, options) {
    try {
        const now = new Date();
...
export function formatDuration(seconds, language, options) {
    try {
        ...
        for (const { unit, seconds: unitSeconds } of units) {
            ...
            if (value > 0) {
                ...
                const unitLabel = unitFormatter.formatToParts(1, unit)
...
export function formatFileSize(bytes, language, options) {
    try {
...
class EnhancedBackend {
    constructor() {
        Object.defineProperty(this, "loadPaths", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
...
Object.defineProperty(EnhancedBackend, "type", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 'backend'
    ...
    return i18n.init({
        ...
        if (appConfig.preloadNamespaces && appConfig.preloadNamespaces.length > 0) {
            try {
                ...
                await translationLoader.preloadNamespaces(initialLanguage, appConfig.preloadNamespaces, loadPath);
...
export const initI18nWithConfig = (config) => initI18n(config);
