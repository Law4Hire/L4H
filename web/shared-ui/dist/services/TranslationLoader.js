...
export class TranslationLoader {
     ...
     */
    async preloadNamespaces(language, namespaces, loadPath) {
        const results = {};
     ...
     */
    async loadMultipleTranslations(requests, priorityNamespaces = ['common', 'errors']) {
        const results = {};
        ...
        const lazyRequests = requests.map(req => ({
            ...
            priority: req.priority || (priorityNamespaces.includes(req.namespace) ? 'critical' : 'normal')
        }));
        try {
            ...
            const priorityRequests = requests.filter(req => priorityNamespaces.includes(req.namespace));
            const regularRequests = requests.filter(req => !priorityNamespaces.includes(req.namespace));
    ...
    async performLoadWithRetry(options, startTime) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (options.offline !== false && navigator.onLine) {
                    offlineTranslationManager.storeOffline(options.language, options.namespace, data).catch(error => {
                ...
                lastError = error;
                // Record error
                translationErrorHandler.recordError(options.language, options.namespace, lastError);
                ...
                console.warn(`Translation load attempt ${attempt + 1} failed for ${options.language}/${options.namespace}. ` +
                    `Retrying in ${Math.round(totalDelay)}ms...`, lastError);
    ...
    async performSingleLoad(options) {
    ...
    buildLoadUrl(loadPath, language, namespace) {
        return loadPath
    ...
    cacheTranslation(language, namespace, data) {
