...
export function useTranslationPerformance(options = {}) {
    const { enableRealTimeMonitoring = true, alertThreshold = 'medium', updateInterval = 30000, // 30 seconds
    autoOptimize = false, optimizationInterval = 300000 // 5 minutes
     } = options;
    ...
    const alertUnsubscribeRef = useRef(null);
    ...
    const initialize = useCallback(async (application, language, loadPaths) => {
        setState(prev => ({ ...prev, isLoading: true, error: null }));
        try {
            const result = await translationPerformanceIntegration.initialize(application, language, loadPaths);
            if (result.success) {
                setState(prev => ({
                    ...prev,
                    isInitialized: true,
...
export function useTranslationPerformanceAlerts(severity = 'medium') {
