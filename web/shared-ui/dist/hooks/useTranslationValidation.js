...
import { translationValidator } from '../services/TranslationValidator';
import { translationLoader } from '../services/TranslationLoader';
export function useTranslationValidation(options = {}) {
    const { i18n } = useTranslation();
    const { languages = ['en-US', 'es-ES', 'fr-FR', 'de-DE'], namespaces = ['common', 'errors', 'forms'], referenceLanguage = 'en-US', autoValidate = false, validationInterval = 300000 // 5 minutes
     } = options;
     ...
     */
    const loadTranslationData = useCallback(async (language, namespace) => {
        try {
    ...
    const validateTranslations = useCallback(async () => {
        ...
        try {
            ...
            for (const language of languages) {
                ...
                const validationResult = translationValidator.validateCompleteness(combinedTarget, combinedReference, language);
                validationResults[language] = validationResult;
                // Generate completeness report
                const completenessReport = translationValidator.generateCompletenessReport(combinedTarget, combinedReference, language);
            ...
            const missingKeysReport = translationValidator.generateMissingKeysReport(allTranslations, referenceLanguage);
            // Calculate overall error/warning state
            const hasErrors = Object.values(validationResults).some(result => result.errors.some(error => error.severity === 'error'));
            const hasWarnings = Object.values(validationResults).some(result => result.warnings.length > 0 || result.errors.some(error => error.severity === 'warning'));
            setState(prev => ({
                ...prev,
                isValidating: false,
                validationResults,
     ...
     */
    const validateLanguage = useCallback(async (language) => {
        if (language === referenceLanguage) {
    ...
    const getValidationSummary = useCallback(() => {
        ...
        const languagesWithErrors = Object.values(validationResults).filter(result => result.errors.some(error => error.severity === 'error')).length;
        const languagesWithWarnings = Object.values(validationResults).filter(result => result.warnings.length > 0).length;
        const averageCompleteness = completenessReports.length > 0
            ...
            : 0;
        const totalMissingKeys = Object.values(state.missingKeysReport).reduce((sum, report) => sum + (report.count || 0), 0);
        return {
            totalLanguages,
            languagesWithErrors,
     ...
     */
    const getMissingKeys = useCallback((language) => {
     ...
     */
    const getValidationErrors = useCallback((language) => {
        const validationResult = state.validationResults[language];
     ...
     */
    const checkKeyExistence = useCallback(async (key) => {
        const results = {};
        for (const language of languages) {
            try {
